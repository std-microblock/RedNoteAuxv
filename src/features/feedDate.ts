...
export const feedDateSort: Feature = {
    ...
    enable(cfg) { return cfg.feedSortByDateAsc; },
    install() {
...
export const feedShowDate: Feature = {
    key: "feedShowDate",
    enable(cfg) { return cfg.feedShowDate; },
    install() {
        hookMultiTypeAdapterSetItems();
    },
};
...
function hookMultiTypeAdapterSetItems() {
    ...
    if (!Adapter) { warn("[feed] MultiTypeAdapter not found"); return; }
    try {
...
function maybeRewriteFeed(list: any): any | null {
    ...
    try {
        ...
        log("[feed] setItems size=" + size + " firstType=" + cn + " isNote=" + isInstanceOf(first, NoteItemBean));
    } catch { /* ignore */ }
...
function parseTime(bean: any): number {
    try {
        ...
        if (ts) { const n = Number(ts); if (!isNaN(n)) return n; }
        const ct = strField(bean, "createTime");
        if (ct) { const n = Number(ct); if (!isNaN(n)) return n; }
        const pt = strField(bean, "postTime");
        if (pt) { const n = Number(pt); if (!isNaN(n)) return n; }
    } catch { /* ignore */ }
    return 0;
}
...
function formatTime(tsSeconds: number): string {
    try {
        ...
        const sdf = SimpleDateFormat.$new(Java.use("java.lang.String").$new("MM-dd HH:mm"));
